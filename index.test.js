const Digisigner = require('./dist/index');
const dotenv = require('dotenv');
require('isomorphic-fetch');

const Document = require('./dist/Document');
const Existing_Field = require('./dist/Existing_Field');
const Signature_Request = require('./dist/Signature_Request');

dotenv.config();

describe('Digisigner class', () => {

  it('is gets a document', () => {
    let digisigner = new Digisigner(process.env.DIGISIGNER_API_KEY);

    return digisigner.downloadDocument(process.env.DIGISIGNER_DOCUMENT_ID)
      .then(res => {
        expect(res).toHaveProperty('success', true);
      });
  });
  
  it ('creates gets a signature request', () => {
    expect.assertions(1); // Necessary for Promises

    let digisigner = new Digisigner(process.env.DIGISIGNER_API_KEY);

    let signer = new Signer({
      email: 'heyheyguy@mailinator.com',
      role: 'Signer 1',
      existing_fields: [
        // provider_full_name_2
        new Existing_Field({
          'api_id': '470e385d-66f4-43c5-81fe-3156ebe641bb',
          'content': 'Stephen Orr',
          'read_only': true
        }),
        // cell_number
        new Existing_Field({
          "api_id": "dfa75dbd-e2a6-4f8e-aa51-d89367d5df6d",
          "content": "3143086391",
          "read_only": false
        }),
        // email
        new Existing_Field({
          "api_id": "a38cdfd5-8da5-43dc-b295-9465668134ab",
          "content": "stephen@fyresite.com",
          "read_only": false
        }),
        // address_2
        new Existing_Field({
          "api_id": "ee47e83d-db29-4658-91e0-a65a92969618",
          "content": "Apt 24",
          "label": "address_2",
          "read_only": false
        }),
        // address_1
        new Existing_Field({
          "api_id": "273b89db-7b93-4ea7-8fd6-62dc75c03650",
          "content": "123 Fake St.",
          "label": "address_1",
          "read_only": false
        }),
        // name_authorized_person
        new Existing_Field({
          "api_id": "6acd8932-5928-4e98-ac79-fd969ff7f2dc",
          "content": "Stephen Orr",
          "read_only": false
        }),
        // provider_full_name
        new Existing_Field({
          "api_id": "526ae178-c7c9-4cab-b768-83fcbb26f657",
          "content": "Stephen Orr",
          "read_only": false
        }),
        // month
        new Existing_Field({
          "api_id": "8a8ccad7-250d-402f-b564-d1bd7d6f5d51",
          "content": "February",
          "read_only": false
        }),
        // day
        new Existing_Field({
          "api_id": "eae5fec4-6be9-4572-a7fe-175f0ae457fa",
          "content": "27",
          "read_only": false
        })
      ]
    });

    let document = new Document({
      document_id: process.env.DIGISIGNER_DOCUMENT_ID,
      signers: [signer]
    });

    let signature_request = new Signature_Request({
      redirect_after_signing_to_url: 'https://google.com'
    });

    signature_request.addDocument(document);

    return digisigner.sendSignatureRequest(signature_request)
      .then(res => {
        // console.log(res.response);
        // console.log(res.response.documents[0].signers[0]);
        expect(res).toHaveProperty('success', true);
      });
  });
});
